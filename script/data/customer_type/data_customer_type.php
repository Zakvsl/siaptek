...
<?php
  ...
  $array_field=array("custtp_code", "custtp_name", "custtp_is_active");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_customer_type="SELECT custtp_id, custtp_code, custtp_name, 
			...
			$q_page="SELECT count(custtp_id) as total_page 
	                 FROM customer_type
					 WHERE $find_data";  	 
		  }
	 }
...
<form id="f_customer_type" name="f_customer_type" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="61%" scope="col"><h2>DATA TIPE CUSTOMER</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
						         if ($fields=='custtp_code')
								     echo "Kode Tipe Customer";
								 else
								 if ($fields=='custtp_name')
								     echo "Nama Tipe Customer";
								 else
								 if ($fields=='custtp_is_active')
								     echo "Status";     
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="custtp_code">Kode Tipe Customer</option>
		  <option value="custtp_name">Nama Tipe Customer</option>
		  <option value="custtp_is_active">Status</option>
        </select>
		<?php } 
      ...
      <td width="23%" align="right" nowrap="nowrap">Sorted by   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
									          if ($field=='custtp_code')
								                  echo "Kode Tipe Customer";
								              else
								              if ($field=='custtp_name')
								                  echo "Nama Tipe Customer";            
								              else
								              if ($field=='custtp_is_active')
								                  echo "Status";     
											?>
          </option>
          <?php    
...
<td width="16%" align="left"><?php 
                              if ($customer_type_autho=='W' || $customer_type_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="17%" scope="col" class="th_ltb">Kode Tipe Customer</th>
          <th width="33%" scope="col" class="th_ltb">Nama Tipe Customer</th>
          <th width="45%" scope="col" class="th_ltb">Status</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection,$q_show_customer_type);
				  $total_customer_type=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_customer_type=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_customer_type['custtp_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_customer_type['custtp_code'];?></td>
          <td class="td_lb"><?php echo $data_customer_type['custtp_name'];?></td>
          <td class="td_lb"><?php if ($data_customer_type['custtp_is_active']=='InActive')
		                             {
							...
							?>
		                    <input type="checkbox" id="chk_is_active_<?php echo $data_customer_type['custtp_id'];?>" 
							                       name="chk_is_active_<?php echo $data_customer_type['custtp_id'];?>" 
												   value='1' 
												   <?php echo $checked; echo $disabled;?>
												   onclick="call_in_active('<?php echo $data_customer_type['custtp_id'];?>')"/><?php echo $data_customer_type['custtp_is_active'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/customer_type/cru_customer_type.php?c=i", "f_cru_customer_type", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data(a)
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    if (a=='1')
				        open_child=window.open('../data/customer_type/cru_customer_type.php?c=u&id='+value_id, 'f_cru_customer_type', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
					else
					    open_child=window.open('../data/customer_type/pic_customer_type.php?id='+value_id, 'f_pic_customer_type', 'scrollBars=Yes, height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_customer_type.action="../data/customer_type/delete_customer_type.php";
					   document.f_customer_type.submit();
					 }  
				}
		   }   
	...
	function show_hide()
	         {
			   ...
			   if (x=='custtp_is_active')
			       document.getElementById('div_1').style.display='inline';
			   else
			       document.getElementById('div_1').style.display='none';
			 }
	
	function call_in_active(id)
	         {
			   var confirmation=confirm('Apakah yakin akan di InActivekan?');
			   var chk='chk_is_active_'+id;
			   if (confirmation)
			      {
				    document.f_customer_type.action="../data/customer_type/in_active.php?id="+id;
					document.f_customer_type.submit();
				  }
			   else
