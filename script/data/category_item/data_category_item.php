...
<?php
  ...
  $array_field=array("cati_code", "cati_name", "cati_notes");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_category_item="SELECT cati_id, cati_code, cati_name, cati_notes
                                   FROM category_item
							       WHERE $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(cati_id) as total_page 
	                 FROM category_item
					 WHERE $find_data";  	 
		  }
	 }
  ...
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_category_item="SELECT cati_id, cati_code, cati_name, cati_notes
                                   FROM category_item $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(cati_id) as total_page 
	                 FROM category_item"; 
		  }
	   else 
  	 ...
  	 {
	   $q_show_category_item="SELECT cati_id, cati_code, cati_name, cati_notes
                              FROM category_item $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(cati_id) as total_page 
	            FROM category_item"; 
	 }	

 //echo $q_show_category_item."<br>";	
 //echo $q_page; 
...
<form id="f_category_item" name="f_category_item" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DAFTAR KATEGORI ASET</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field">
		  <option value="cati_code">Kode Kategori</option>
		  <option value="cati_name">Nama Kategori</option>
		  <option value="cati_notes">Keterangan</option>
        </select>
		<?php } 
...
<td width="9%" align="left"><?php 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="18%" scope="col" class="th_ltb">Kode Kategori</th>
          <th width="31%" scope="col" class="th_ltb">Nama Kategori</th>
          <th width="46%" scope="col" class="th_ltb">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_category_item);
				  $total_category_item=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_category_item=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_category_item['cati_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_category_item['cati_code'];?></td>
          <td class="td_lb"><?php echo $data_category_item['cati_name'];?></td>
          <td class="td_lb"><?php echo $data_category_item['cati_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_category_item.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/category_item/cru_category_item.php?c=i", "f_cru_category_item", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/category_item/cru_category_item.php?c=u&id='+value_id, 'f_cru_category_item', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_category_item.action="../data/category_item/delete_category_item.php";
					   document.f_category_item.submit();
					 }  
				}
		   }   
	...
	function call_export()
	         {
			   ...
			   window.location="../data/category_item/export_to_excel.php?f="+field+"&o="+operator+"&t="+text_to_find;
			 }  
</script>
