...
<?php
  ...
  $array_field=array("cidh_code", "cidh_date", "itemd_code", "masti_name", "itemd_serial_no", "cidd_is_canceled", "cidd_notes");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_position_report="SELECT cidh_code, cidh_date, cidd_id, change_item_description_detail.itemd_id, itemd_code, masti_id_old, masti_id_new,
									 ...
									 WHERE change_item_description_header.branch_id='$branch_id' AND $find_data
  ...
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      ...
	      {	  
	        ...
	        $q_show_position_report="SELECT cidh_code, cidh_date, cidd_id, change_item_description_detail.itemd_id, itemd_code, masti_id_old, masti_id_new,
                                     ...
                                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id 
									 INNER JOIN users ON users.users_id=change_item_description_header.created_by
									 WHERE change_item_description_header.branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row";
			$q_page="SELECT COUNT(*) AS total_page
	                 FROM change_item_description_detail 
                     INNER JOIN change_item_description_header ON change_item_description_header.cidh_id=change_item_description_detail.cidh_id 
                     INNER JOIN item_detail ON item_detail.itemd_id=change_item_description_detail.itemd_id 
	   ...
	   $q_show_position_report="SELECT cidh_code, cidh_date, cidd_id, change_item_description_detail.itemd_id, itemd_code, masti_id_old, masti_id_new,
                                     ...
                                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id 
									 INNER JOIN users ON users.users_id=change_item_description_header.created_by
									 WHERE change_item_description_header.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row";
	   $q_page="SELECT COUNT(*) AS total_page
	            FROM change_item_description_detail 
                INNER JOIN change_item_description_header ON change_item_description_header.cidh_id=change_item_description_detail.cidh_id 
                INNER JOIN item_detail ON item_detail.itemd_id=change_item_description_detail.itemd_id 
...
<form id="f_item_detail" name="f_item_detail" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>LAPORAN PERUBAHAN DESKRIPSI ISI ASET </h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='cidd_is_canceled')
								     echo "Dibatalkan"; 
								 else
								 if ($fields=='cidd_notes')
								     echo "Keterangan";  
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="cidd_is_canceled">Dibatalkan</option>
		  <option value="cidd_notes">Keterangan</option>
        </select>
		<?php } 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="15%" scope="col" class="th_ltb">Deskripsi Isi Aset Sebelumnya </th>
          <th width="15%" scope="col" class="th_ltb">Deskripsi Isi Aset Baru </th>
          <th width="7%" scope="col" class="th_ltb">Serial No </th>
          <th width="6%" scope="col" class="th_ltb">Dibatalkan</th>
          <th width="8%" scope="col" class="th_ltb">Dibuat Oleh </th>
          <th width="18%" scope="col" class="th_ltb">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
        ...
        <tr>
          ...
          <td class="td_lb"><?php echo $data_item_detail['cidh_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_item_detail['cidh_date']);?></td>
          <td class="td_lb"><?php echo $data_item_detail['itemd_code'];?></td>
          <td class="td_lb"><?php echo $data_item_detail['masti_name'];?></td>
          <td class="td_lb"><?php echo $data_item_detail['masti_name_1'];?></td>
          <td class="td_lb"><?php echo $data_item_detail['itemd_serial_no'];?></td>
          <td class="td_lb"><?php if ($data_item_detail['cidd_is_canceled']=='0') 
		                              echo 'Tidak'; 
								  else 
								      echo 'Ya';?></td>
          <td class="td_lb"><?php echo $data_item_detail['users_names'];?></td>
          <td class="td_lb"><?php echo $data_item_detail['cidd_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
			   ...
			   if (field=='cidh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/change_description_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/change_description_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/change_description_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 }  
