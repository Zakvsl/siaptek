...
<?php
  ...
  $array_field=array("disph_code", "disph_date", "brokh_code", "cust_name", "itemd_code", "masti_name", "itemd_serial_no", "cati_name", "dispd_is_canceled");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_dispossal_report="SELECT disph_code, disph_date, brokh_code, cust_name, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, 
                                      itemd_qty,  cati_name, 
                                      ...
                                      WHERE dispossal_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' AND $find_data LIMIT $first_data,$row";
			$q_page="SELECT count(*) as total_page 
                     ...
                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                     INNER JOIN customer ON customer.cust_id=dispossal_header.cust_id
                     WHERE dispossal_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      ...
	      {	  
	        ...
	        $q_show_dispossal_report="SELECT disph_code, disph_date, brokh_code, cust_name, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name,  
                                      ...
                                      INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                                      INNER JOIN customer ON customer.cust_id=dispossal_header.cust_id
                                      WHERE dispossal_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(*) as total_page 
	                 FROM dispossal_detail
                     INNER JOIN dispossal_header ON dispossal_header.disph_id=dispossal_detail.disph_id
                     LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
	   ...
	   $q_show_dispossal_report="SELECT disph_code, disph_date, brokh_code, cust_name, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, 
                                 ...
                                 INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                                 INNER JOIN customer ON customer.cust_id=dispossal_header.cust_id
                                 WHERE  dispossal_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(*) as total_page 
	            FROM dispossal_detail
                INNER JOIN dispossal_header ON dispossal_header.disph_id=dispossal_detail.disph_id
                LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
...
// echo $q_show_dispossal_report."<br>";	
// echo $q_page; 
...
<form id="f_dispossal_report" name="f_dispossal_report" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>LAPORAN PENJUALAN ASET </h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="dispd_is_canceled">Dibatalkan</option>
        </select>
		<?php } 
...
<td width="9%" align="left"><div id="action" align="left">
                              <ul id="nav">
                                <li><a href="" title="Retrive Dispossal Report" onclick="call_export()">Ekspor Data</a></li>
                              </ul>
                            </div></td>
    </tr>
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="12%" scope="col" class="th_ltb">No Ref  Kerusakan </th>
          <th width="14%" scope="col" class="th_ltb">Nama Customer/Vendor </th>
		  ...
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection,$q_show_dispossal_report);
				  $total_dispossal_report=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_dispossal_report=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['disph_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_dispossal_report['disph_date']);?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['brokh_code'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['cust_name'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['itemd_code'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['masti_name'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['itemd_serial_no'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['itemd_capacity']." ".$data_dispossal_report['uom_name'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['itemd_qty']." Cylinder";?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['cati_name'];?></td>
          <td class="td_lb"><?php echo $data_dispossal_report['dispd_is_canceled'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_dispossal_report.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/dispossal_report/cru_dispossal_report.php?c=i", "f_cru_dispossal_report", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/dispossal_report/cru_dispossal_report.php?c=u&id='+value_id, 'f_cru_dispossal_report', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_dispossal_report.action="../data/dispossal_report/delete_dispossal_report.php";
					   document.f_dispossal_report.submit();
					 }  
				}
		   }   
  ...
  function active_inactive()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_dispossal_report.action="../data/dispossal_report/active_invactive_item.php";
					   document.f_dispossal_report.submit();
					 }  
				}
		   }
			   ...
			   if (field=='disph_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/dispossal_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/dispossal_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/dispossal_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 } 
