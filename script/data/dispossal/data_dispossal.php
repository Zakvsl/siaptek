...
<?php
  ...
  $array_field=array("disph_code", "disph_date", "disph_sources", "brokh_code", "cust_name", "disph_is_canceled", "disph_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='disph_code')
	       $field_to_sort='disph_id';
	   else
  ...
  if (isset($_POST['btn_find']))
	 {
	   ...
	   if ($field=='disph_date' && $operator=='between' && ($text_to_find_2<$text_to_find_1))
	      {
			 $continue='1';
			 ?>
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_dispossal="SELECT disph_id, disph_code, disph_date, 
                               CASE disph_sources
                               ...
                               WHERE dispossal_header.branch_id='$branch_id' AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(disph_id) as total_page 
	                 FROM dispossal_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
					 LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
                     WHERE dispossal_header.branch_id='$branch_id'  AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_dispossal="SELECT disph_id, disph_code, disph_date, 
                               ...
                               LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
							   LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
                               WHERE dispossal_header.branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(disph_id) as total_page 
	                 FROM dispossal_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
					 LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
	      ...
	      {	  
	        ...
	        $q_show_dispossal="SELECT disph_id, disph_code, disph_date, 
                               ...
                               LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
							   LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
                               WHERE dispossal_header.branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(disph_id) as total_page 
	                 FROM dispossal_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
					 LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
	   ...
	   $q_show_dispossal="SELECT disph_id, disph_code, disph_date, 
                          ...
                          LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
						  LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
                          WHERE dispossal_header.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(disph_id) as total_page 
	            FROM dispossal_header
                LEFT JOIN broken_header ON broken_header.brokh_id=dispossal_header.brokh_id AND broken_header.branch_id='$branch_id'
				LEFT JOIN customer ON customer.cust_id=dispossal_header.cust_id AND customer.branch_id='$branch_id'
...
<form id="f_dispossal" name="f_dispossal" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA PENJUALAN ASET</h2></td>
      ...
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
						         if ($fields=='disph_code')
								     echo "No Transaksi";
								 else
								 if ($fields=='disph_date')
								     echo "Tanggal";
								 else
								 if ($fields=='disph_sources')
								     echo "Sumber Transaksi";
								 else
								 if ($fields=='brokh_code')
								     echo "No Ref Kerusakan";             
								 else
								 if ($fields=='cust_name')
								     echo "Nama Customer";
								 else
								 if ($fields=='disph_is_canceled')
								     echo "Dibatalkan";
								 else
								 if ($fields=='disph_notes')
								     echo "Keterangan";
						   ?>			 
						     </option>
						  <?php	 
		...
		?>
						  <select name="s_field" id="s_field" onchange="show_hide()">
                            <option value="disph_code">No Transaksi</option>
                            <option value="disph_date">Tanggal</option>
                            <option value="disph_sources">Sumber Transaksi</option>
                            <option value="brokh_code">No Ref Kerusakan</option>
                            <option value="cust_name">Nama Customer</option>
                            <option value="disph_is_canceled">Dibatalkan</option>
                            <option value="disph_notes">Keterangan</option>
                          </select>
		<?php } 
		...
		<?php }?>
        <div id="div_1" style="display:<?php if ($field!='disph_date') echo "inline"; else echo "none";?>"><input type="text" name="txt_find_1" id="txt_find_1"
		 value="<?php 
		         if (isset($_POST['txt_find_1']))
				     ...
				     echo "";	    
		       ?>"></div>
        <div id="div_2" style="display:<?php if ($field=='disph_date') echo 'inline'; else echo 'none';?>">
		   <input name="txt_date_1" type="text" id="txt_date_1" readonly="readonly" size="10" 
											   ...
											   ?>/></div>
		<div id="div_3" style="display:<?php if ($field=='disph_date' && $operator=='between') echo 'inline'; else echo 'none';?>">
		       <input name="txt_date_2" type="text" id="txt_date_2"  readonly="readonly" size="10" 
		                                       <?php 
											      if (isset($_POST['txt_date_2']))
											          ...
											          echo "value='$current_date'";
											   ?>/></div>
        <input type="submit" name="btn_find" id="btn_find" value="Cari"/><div id="div_4" style="display:none">&nbsp;0=Tidak, 1=Ya</div><div id="div_5" style="display:none">&nbsp;0=Internal, 1=Kerusakan</div>
      </div></td>
      <td width="21%" nowrap="nowrap">Urut Berdasarkan   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='disph_is_canceled')
								                  echo "Dibatalkan";
								              else
								              if ($field=='disph_notes')
								                  echo "Keterangan";
											?>
          </option>
          <?php    
...
<td width="9%" align="left"><?php 
                              if ($dispossal_autho=='W' || $dispossal_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="11%" scope="col" class="th_ltb">Sumber Transaksi </th>
          <th width="13%" scope="col" class="th_ltb">No Ref  Kerusakan </th>
          <th width="20%" scope="col" class="th_ltb">Nama Customer</th>
          <th width="7%" scope="col" class="th_ltbr">Dibatalkan</th>
          <th width="22%" scope="col" class="th_ltbr">Ketererangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection,$q_show_dispossal);
				  $total_dispossal=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_dispossal=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_dispossal['disph_id'];?>"/></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><label style="cursor: pointer;" onclick="show_data('<?php echo $data_dispossal['disph_id'];?>')"><font color="#0000FF"><?php echo $data_dispossal['disph_code'];?></font></label></td>
          <td class="td_lb"><?php echo get_date_1($data_dispossal['disph_date']);?></td>
          <td class="td_lb"><?php echo $data_dispossal['disph_sources'];?></td>
          <td class="td_lb"><?php echo $data_dispossal['brokh_code'];?></td>
          <td class="td_lb"><?php echo $data_dispossal['cust_name'];?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_disph_is_canceled_<?php echo $data_dispossal['disph_id'];?>" name="cb_disph_is_canceled_<?php echo $data_dispossal['disph_id'];?>" <?php if ($data_dispossal['disph_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($dispossal_autho!='D') echo "disabled='disabled'";?> onclick="cancel_dispossal('<?php echo $data_dispossal['disph_id'];?>')"><?php if ($data_dispossal['disph_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_dispossal['disph_id'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_dispossal['disph_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_dispossal.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/dispossal/cru_dispossal.php?c=i", "f_cru_dispossal", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/dispossal/cru_dispossal.php?c=u&id='+value_id, 'f_cru_dispossal', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 } 
  ...
  function show_data(id)
           {
			 ...
			 var l=(screen.width/2)-(w/2);
			 var t=(screen.height/2)-(h/2);
		     open_child=window.open('../data/dispossal/cru_dispossal.php?c=d&id='+id+'&b='+branch_id_data, 'f_cru_dispossal', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes'); 
		   } 
	
	function cancel_dispossal(x)
	         {
			    var branch_id_data=document.getElementById('s_branch_data').value;
			    var w=400;
			    var h=230;
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
				var a="cb_disph_is_canceled_"+x;
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/dispossal/canceled_dispossal.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_dispossal", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/dispossal/canceled_dispossal.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_dispossal", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='disph_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/dispossal/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/dispossal/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/dispossal/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 }  
