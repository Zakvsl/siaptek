...
<?php
  ...
  $array_field=array("rth_code", "tth_code", "rth_date", "rth_ba_no", "rth_po_no", "branch_name", "rth_is_canceled", "rth_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='rth_code')
	       $field_to_sort='rth_id';
	   else
  ...
  if (isset($_POST['btn_find']))
	 {
	   ...
	   if ($field=='rth_date' && $operator=='between' && ($text_to_find_2<$text_to_find_1))
	      {
			 $continue='1';
			 ?>
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_receipt_transfer="SELECT rth_id AS rth_id_1, rth_code, tth_code, rth_date, branch_name, rth_ba_no, rth_po_no,
                                      ...
                                      WHERE receipt_transfer_header.branch_id='$branch_id' AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(rth_id) as total_page 
	                 FROM receipt_transfer_header
					 INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
					 INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
                     WHERE receipt_transfer_header.branch_id='$branch_id' AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_receipt_transfer="SELECT rth_id AS rth_id_1, rth_code, tth_code, rth_date, branch_name, rth_ba_no, rth_po_no,
                                      ...
                                      INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
									  INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
                                      WHERE receipt_transfer_header.branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(rth_id) as total_page
			         FROM receipt_transfer_header   
	                 INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
					 INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
	      ...
	      {	  
	        ...
	        $q_show_receipt_transfer="SELECT rth_id AS rth_id_1, rth_code, tth_code, rth_date, branch_name, rth_ba_no, rth_po_no,
                                      ...
                                      INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
									  INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
                                      WHERE receipt_transfer_header.branch_id='$branch_id' AND $find_data $sorting 
	   ...
	   $q_show_receipt_transfer="SELECT rth_id AS rth_id_1, rth_code, tth_code, rth_date, branch_name, rth_ba_no, rth_po_no,
                                 ...
                                 INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
							     INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
                                 WHERE receipt_transfer_header.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(rth_id) as total_page 
	            FROM receipt_transfer_header
                INNER JOIN transfer_header ON transfer_header.tth_id=receipt_transfer_header.tth_id 
				INNER JOIN branch ON branch.branch_id=receipt_transfer_header.branch_id_from
 ...
 // echo $q_show_receipt_transfer."<br>";	
 // echo $q_page; 
?>
<style>
            #action {
...
<form id="f_receipt_transfer" name="f_receipt_transfer" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA PENERIMAAN TRANSFER ASET</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='branch_name')
								     echo "Kantor Cabang Asal";             
								 else
								 if ($fields=='rth_is_canceled')
								     echo "Dibatalkan";             
								 else
								 if ($fields=='rth_notes')
								     echo "Keterangan";
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="rth_code">No Transaksi</option>
		  <option value="tth_code">No Referensi</option>
		  <option value="rth_date">Tanggal</option>
		  <option value="rth_ba_no">Nomor Berita Acara</option>
		  <option value="rth_po_no">Purchase No</option>
		  <option value="branch_name">Kantor Cabang Asal</option>
		  <option value="rth_is_canceled">Dibatalkan</option>
		  <option value="rth_notes">Keterangan</option>
        </select>
		<?php } 
		...
		?>
		 ...
		 <?php }?><div id="div_1" style="display:<?php if ($field!='reth_date') echo "inline"; else echo "none";?>"><input type="text" name="txt_find_1" id="txt_find_1"
		 value="<?php  
...
<td width="9%" align="left"><?php 
                              if ($returning_autho=='W' || $returning_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="14%" scope="col" class="th_ltb">No Referensi</th>
          <th width="7%" scope="col" class="th_ltb">Tanggal</th>
          <th width="11%" scope="col" class="th_ltb">Nomor Berita Acara </th>
          <th width="11%" scope="col" class="th_ltb">Purchase No </th>
          <th width="11%" scope="col" class="th_ltb">Kantor Cabang Asal</th>
          <th width="8%" scope="col" class="th_ltbr">Total Aset </th>
          <th width="6%" scope="col" class="th_ltbr">Dibatalkan</th>
          <th width="14%" scope="col" class="th_ltbr">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_receipt_transfer);
				  $total_receipt_transfer=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_receipt_transfer=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_receipt_transfer['rth_id_1'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><label style="cursor: pointer;" onclick="show_data('<?php echo $data_receipt_transfer['rth_id_1'];?>')"><font color="#0000FF"><?php echo $data_receipt_transfer['rth_code'];?></font></label></td>
          <td class="td_lb"><?php echo $data_receipt_transfer['tth_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_receipt_transfer['rth_date']);?></td>
          <td class="td_lb"><?php echo $data_receipt_transfer['rth_ba_no'];?></td>
          <td class="td_lb"><?php echo $data_receipt_transfer['rth_po_no'];?></td>
          <td class="td_lb"><?php echo $data_receipt_transfer['branch_name'];?></td>
          <td class="td_lbr"><?php echo $data_receipt_transfer['total_cylinder'];?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_reth_is_canceled_<?php echo $data_receipt_transfer['rth_id_1'];?>" name="cb_reth_is_canceled_<?php echo $data_receipt_transfer['rth_id_1'];?>" <?php if ($data_receipt_transfer['rth_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($receipt_transfer_autho!='D') echo "disabled='disabled'";?> onclick="cancel_receipt_transfer('<?php echo $data_receipt_transfer['rth_id_1'];?>')"><?php if ($data_receipt_transfer['rth_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_receipt_transfer['rth_id_1'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_receipt_transfer['rth_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_receipt_transfer.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/receipt_transfer/cru_receipt_transfer.php?c=i&b="+branch_id_data, "f_cru_receipt_transfer", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/receipt_transfer/cru_receipt_transfer.php?c=u&id='+value_id+'&b='+branch_id_data, 'f_cru_receipt_transfer', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 }  
  ...
  function show_data(id)
           {
			 ...
			 var l=(screen.width/2)-(w/2);
			 var t=(screen.height/2)-(h/2);
		     open_child=window.open('../data/receipt_transfer/cru_receipt_transfer.php?c=d&id='+id+'&b='+branch_id_data, 'f_cru_receipt_transfer', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
		   }
	
	function cancel_receipt_transfer(x)
	         {
			    var branch_id_data=document.getElementById('s_branch_data').value;
			    var w=400;
			    var h=230;
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
				var a="cb_reth_is_canceled_"+x;
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/receipt_transfer/canceled_receipt_transfer.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_receipt_transfer", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/receipt_transfer/canceled_receipt_transfer.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_receipt_transfer", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='reth_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/receipt_transfer/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2+"&b="+branch_id_data);
					   }
					else
					   open_child=window.open("../data/receipt_transfer/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&b="+branch_id_data); 
				  }
			   else 
			      open_child=window.open("../data/receipt_transfer/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&b="+branch_id_data); 
			 }  
