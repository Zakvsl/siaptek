  ...
  if ($cru=='u')
     {
	   ...
	   $q_get_customer="SELECT cust_id, cust_code, cust_name, cust_npwp, cust_address, cust_status, cust_phone, cust_fax, cust_web_address, cust_email, custtp_id, cust_type
                        FROM customer
				        WHERE cust_id='$id' AND cust_type='0' AND branch_id='$branch_id'";
	//   echo $q_get_customer;
	   ...
	   if ($total_customer==0)
	      {
		    ?>
			   <script language="javascript">
		         alert('customer yang akan diupdate tidak ditemukan!');
				 window.close();
			   </script>
		    <?php	
		  }
  ...
  <table width="403" border="0" cellspacing="1" cellpadding="1">
    ...
    <tr>
      <th colspan="3" scope="col"><div align="left"><?php if ($cru=='i') 
	                                                         {
															   echo "TAMBAH DATA CUSTOMER BARU";
															 }
														  else
														  if ($cru=='u')
														     {
															   echo "UBAH DATA CUSTOMER";
															 }
													?>	  </th>
    </tr>
    <tr>
      <td width="33%" nowrap="nowrap">Kode Customer</td>
      <td width="1%">:</td>
      <td width="66%"><label>
    ...
    <tr>
      <td>Tipe Customer</td>
      <td>:</td>
      <td><select id="s_customer_type" name="s_customer_type" style="width:200px">
		   <option value="0">-Pilih Tipe Customer-</option>
	        <?php
			 $selected=""; 
	         $q_get_customer_type="SELECT * FROM customer_type WHERE custtp_is_active='0'";
			 $exec_get_customer_type=mysqli_query($db_connection,$q_get_customer_type);
		     while ($field_data=mysqli_fetch_array($exec_get_customer_type))
		           {
				     if ($custtp_id==$field_data['custtp_id']) 
					     $selected="selected='selected'";
				     else
					     $selected="";
					 echo "<option value='".$field_data['custtp_id']."' $selected>".$field_data['custtp_code']." - ".$field_data['custtp_name']."</option>";
				   } 	 
		    ?>
        </select>
      </td>
  ...
  if (isset($_POST['btn_save']))
     {
	   ...
	   $cust_type=$_POST['s_customer_type'];
	   ...
	   $q_check_tipe_customer="SELECT * FROM customer_type WHERE custtp_id='$cust_type'";
	   $exec_check_tipe_customer=mysqli_query($db_connection,$q_check_tipe_customer);
	   $field_tipe=mysqli_fetch_array($exec_check_tipe_customer);
	   ...
	   if (trim($cust_type=='0'))
	      {
		    ?>
			  <script language="javascript">
			    alert('Tipe customer harus diisi!');
				window.location.href='javascript:history.back(1)';
			  </script>
			<?php 
		  }
	   else
	   if ($field_tipe['custtp_is_active']=='1')
	      {
		    ?>
			  <script language="javascript">
			    alert('Status Tipe Customer sudah InActive!');
				window.location.href='javascript:history.back(1)';
			  </script>
			<?php 
		  }
	   ...
	   if ($cust_id=='')  //jika tambah data
	      {
		    $q_check_customer="select * from customer where cust_code='$cust_code' AND cust_type='0' AND branch_id='$branch_id'";
			$exec_check_customer=mysqli_query($db_connection,$q_check_customer);
			if (mysqli_num_rows($exec_check_customer)>0)
			   {
			     ?>
			       <script language="javascript">
		             alert('Duplikasi Kode customer!');
					 window.location.href='javascript:history.back(1)';
			       </script>
		         <?php 
			   }
	      ...
	      { 	  
		    /*
		    $q_check_customer_issuing="SELECT * FROM issuing_header WHERE issuingh_type='0' and cust_id='$cust_id'";
		//	echo $q_check_customer_issuing;
			...
			if (mysqli_num_rows($exec_check_customer_issuing)>0)
			   ...
			   {   
			     */
  		         $q_check_customer="select * from customer where cust_id='$cust_id' and cust_type='0' AND branch_id='$branch_id'";
			     $exec_check_customer=mysqli_query($db_connection,$q_check_customer);
			     if (mysqli_num_rows($exec_check_customer)>0)
			        {	
			          if ($cust_code==$cust_code_1)
			             {	 
				           ...
				           $q_update_customer="UPDATE customer SET cust_name='$cust_name', cust_npwp='$cust_npwp', cust_address='$cust_address', cust_status='$cust_status',
					     	                          cust_web_address='$cust_web', cust_email='$cust_email', cust_phone='$cust_phone', cust_fax='$cust_fax', custtp_id='$cust_type'
					     	  	               WHERE cust_id='$cust_id' AND branch_id='$branch_id'";
					        ...
					        if (mysqli_num_rows($exec_check_customer)==0)
					           {
				                 ...
				                 $q_update_customer="UPDATE customer SET cust_code='$cust_code', cust_name='$cust_name', cust_npwp='$cust_npwp', cust_status='$cust_status',
						     		                        cust_address='$cust_address',cust_web_address='$cust_web', cust_email='$cust_email', cust_phone='$cust_phone',
 									     	                cust_fax='$cust_fax', custtp_id='$cust_type'
						  	                         WHERE cust_id='$cust_id' AND branch_id='$branch_id'";			
                               //  echo $q_update_customer;
							     $exec_update_customer=mysqli_query($db_connection,$q_update_customer);
				                 if ($exec_update_customer)
