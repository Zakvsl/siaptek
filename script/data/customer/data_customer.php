...
<?php
  ...
  $array_field=array("cust_code", "cust_name", "cust_address", "cust_status", "cust_phone", "cust_fax", "cust_email", "custtp_name");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
			                       CASE cust_status 
                                        WHEN '0' THEN 'Active'
                                        WHEN '1' THEN 'InActive'
                                   END cust_status, cust_phone, cust_fax, cust_email, custtp_name
                              FROM customer
							  LEFT JOIN customer_type ON customer_type.custtp_id=customer.custtp_id
							  WHERE cust_type='0' AND branch_id='$branch_id' AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
					 WHERE cust_type='0' AND branch_id='$branch_id' AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                            ...
                            FROM customer
							LEFT JOIN customer_type ON customer_type.custtp_id=customer.custtp_id
							WHERE cust_type='0' AND branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
	      ...
	      {	  
	        ...
	        $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                            ...
                            FROM customer
							LEFT JOIN customer_type ON customer_type.custtp_id=customer.custtp_id
							WHERE cust_type='0' AND branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
				     WHERE cust_type='0' AND branch_id='$branch_id' AND $find_data";  
		  }			 
	   ...
	   $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                       ...
                       FROM customer
					   LEFT JOIN customer_type ON customer_type.custtp_id=customer.custtp_id
					   WHERE cust_type='0' AND branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(cust_id) as total_page 
	            FROM customer
				WHERE cust_type='0' AND branch_id='$branch_id'"; 
	 }	

 //echo $q_show_customer."<br>";	
 //echo $q_page; 
...
<form id="f_customer" name="f_customer" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA CUSTOMER</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='cust_address')
								     echo "Alamat";             
								 else
								 if ($fields=='cust_status')
								     echo "Status";             
								 else
								 if ($fields=='cust_phone')
								     echo "No Phone";
								 else
								 if ($fields=='cust_fax')
								     echo "Fax No";
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="cust_code">Kode Customer</option>
		  <option value="cust_name">Nama Customer</option>
		  <option value="cust_address">Alamat</option>
		  <option value="cust_status">Status</option>
		  <option value="cust_phone">No Telpon</option>
		  <option value="cust_fax">Fax No</option>
        ...
        <input type="text" name="txt_find_1" id="txt_find_1"
		 value="<?php 
		         if (isset($_POST['txt_find_1']))
				     ...
				     echo "";	    
		       ?>">
        <input type="submit" name="btn_find" id="btn_find" value="Find"/><div id="div_1" style="display:none">&nbsp;0=Active, 1=InActive</div>
      </div></td>
      <td width="21%" align="right">Sorted by   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='cust_address')
								                  echo "Alamat";             
								              else
								              if ($field=='cust_status')
								                  echo "Status";             
								              else
								              if ($field=='cust_phone')
								                  echo "No Telpon";
...
<td width="9%" align="left"><?php 
                              if ($customer_autho=='W' || $customer_autho=='D')
							     { 
								   ...
								   if ($customer_autho=='D')
								      {
								        echo "<li><a href='javascript:void(1)' title='Add New Data' onclick='input_data()'>Tambah Data</a></li>";
										echo "<li><a href='javascript:void(1)' title='Add New PIC' onclick='update_data(0)'>Tambah PIC</a></li>";
		                                echo "<li><a href='javascript:void(1)' title='Update Data' onclick='update_data(1)'>Ubah Data</a></li>";
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="10%" scope="col" class="th_ltb">Kode Customer</th>
          <th width="15%" scope="col" class="th_ltb">Nama Customer</th>
          <th width="22%" scope="col" class="th_ltb">Alamat</th>
          <th width="4%" scope="col" class="th_ltb">Status</th>
          <th width="8%" scope="col" class="th_ltb">No Telpon</th>
          <th width="8%" scope="col" class="th_ltbr">Fax No</th>
          <th width="17%" scope="col" class="th_ltbr">Email</th>
          <th width="17%" scope="col" class="th_ltbr">Tipe Customer</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection,$q_show_customer);
				  $total_customer=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_customer=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_customer['cust_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_customer['cust_code'];?></td>
          <td class="td_lb"><?php echo $data_customer['cust_name'];?></td>
          <td class="td_lb"><?php echo $data_customer['cust_address'];?></td>
          <td class="td_lb"><?php echo $data_customer['cust_status'];?></td>
          <td class="td_lb"><?php echo $data_customer['cust_phone'];?></td>
          <td class="td_lb"><?php echo $data_customer['cust_fax'];?></td>
          <td class="td_lbr"><?php echo $data_customer['cust_email'];?></td>
          <td class="td_lbr"><?php echo $data_customer['custtp_name'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Page : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_customer.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/customer/cru_customer.php?c=i", "f_cru_customer", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data(a)
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    if (a=='1')
				        open_child=window.open('../data/customer/cru_customer.php?c=u&id='+value_id, 'f_cru_customer', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
					else
					    open_child=window.open('../data/customer/pic_customer.php?id='+value_id, 'f_pic_customer', 'scrollBars=Yes, height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_customer.action="../data/customer/delete_customer.php";
	...
	function call_export()
	         {
			   ...
			   open_child=window.open("../data/customer/export_to_excel.php?f="+field+"&o="+operator+"&t="+text_to_find);
			 }  
