...
<?php
  ...
  $array_field=array("brokh_code", "brokh_date", "itemd_code", "masti_name", "itemd_serial_no", "cati_name", "brokd_is_canceled", "brokd_is_wo", "brokd_is_dispossed", "brokd_notes");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_broken_repport="SELECT brokh_code, brokh_date, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, 
                                     itemd_qty, cati_name, 
                                     CASE brokd_is_canceled
                                          WHEN '0' THEN 'Tidak'
                                          WHEN '1' THEN 'Ya'
                                     END brokd_is_canceled, 
									 CASE brokd_is_wo
                                          WHEN '0' THEN 'Tidak'
                                          WHEN '1' THEN 'Ya'
                                     END brokd_is_wo,
                                     CASE brokd_is_dispossed
                                          WHEN '0' THEN 'Tidak'
                                          WHEN '1' THEN 'Ya'
                                     END brokd_is_dispossed, brokd_notes
                                     FROM broken_detail
                                     INNER JOIN broken_header ON broken_header.brokh_id=broken_detail.brokh_id
                                     INNER JOIN item_detail ON broken_detail.itemd_id=item_detail.itemd_id
                                     ...
                                     WHERE broken_header.branch_id='$branch_id' AND $find_data LIMIT $first_data,$row";
			$q_page="SELECT count(*) as total_page 
                     ...
                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                     WHERE broken_header.branch_id='$branch_id' AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_broken_repport="SELECT brokh_code, brokh_date, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name,  
                                     ...
                                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                                     WHERE broken_header.branch_id='$branch_id'
	      ...
	      {	  
	        ...
	        $q_show_broken_repport="SELECT brokh_code, brokh_date, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name,  
                                    itemd_qty, cati_name, 
                                     ...
                                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                                     WHERE broken_header.branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(*) as total_page 
	                 FROM broken_detail
                     INNER JOIN broken_header ON broken_header.brokh_id=broken_detail.brokh_id
                     INNER JOIN item_detail ON broken_detail.itemd_id=item_detail.itemd_id
	   ...
	   $q_show_broken_repport="SELECT brokh_code, brokh_date, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name,  
                                     ...
                                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id
                                     WHERE broken_header.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(*) as total_page 
	            FROM broken_detail
                INNER JOIN broken_header ON broken_header.brokh_id=broken_detail.brokh_id
                INNER JOIN item_detail ON broken_detail.itemd_id=item_detail.itemd_id
...
<form id="f_issuing_repport" name="f_issuing_repport" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>LAPORAN ASET RUSAK </h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='brokd_is_canceled')
								     echo "Dibatalkan"; 
								 else
								 if ($fields=='brokd_is_wo')
								     echo "Dihapus"; 
								 else
								 if ($fields=='brokd_is_dispossed')
								     echo "Dijual"; 
								 else
								 if ($fields=='brokd_notes')
								     echo "Keterangan";
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="brokd_is_canceled">Dibatalkan</option>
		  <option value="brokd_is_wo">Dihapus</option>
		  <option value="brokd_is_dispossed">Dijual</option>
		  <option value="brokd_notes">Keterangan</option>
        </select>
		<?php } 
		       ...
		       <input name="txt_date_2" type="text" id="txt_date_2"  readonly="readonly" size="10" 
		                                       <?php 
											      if (isset($_POST['txt_date_2']))
											          ...
											          echo "value='$current_date'";
											   ?>/></div>
        <input type="submit" name="btn_find" id="btn_find" value="Cari"/><div id="div_4" style="display:none">&nbsp;0=Tidak, 1=Ya</div><div id="div_5" style="display:none">&nbsp;0=Tidak, 1=Ya</div>
      </div></td>
      <td width="21%" align="right" nowrap="nowrap">Urut Berdasarkan    :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='brokd_is_canceled')
								                  echo "Dibatalkan"; 
								              else
								              if ($field=='brokd_is_wo')
								                  echo "Dihapus"; 
											  else
											  if ($field=='brokd_is_dispossed')
								                  echo "Dijual"; 
											  else
								              if ($field=='brokd_notes')
								                  echo "Keterangan";
											?>
          </option>
          <?php    
...
<td width="9%" align="left"><div id="action" align="left">
                              <ul id="nav">
                                <li><a href='' title="Retrive Broken Report" onclick="call_export()">Ekspor Data</a></li>
                              </ul>
                            </div></td>
    </tr>
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="10%" scope="col" class="th_ltb">Kode Aset </th>
          ...
          <th width="5%" scope="col" class="th_ltb">Dijual</th>
          <th width="14%" scope="col" class="th_ltb">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_broken_repport);
				  $total_broken_repport=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_broken_repport=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_broken_repport['brokh_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_broken_repport['brokh_date']);?></td>
          <td class="td_lb"><?php echo $data_broken_repport['itemd_code'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['masti_name'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['itemd_serial_no'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['itemd_capacity']." ".$data_broken_repport['uom_name'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['itemd_qty']." Cylinder";?></td>
          <td class="td_lb"><?php echo $data_broken_repport['cati_name'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['brokd_is_canceled'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['brokd_is_wo'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['brokd_is_dispossed'];?></td>
          <td class="td_lb"><?php echo $data_broken_repport['brokd_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
 ...
 if (isset($_POST['btn_find']) || (isset($_POST['s_sorting_field']) || isset($_POST['s_sort'])))
    {
	  ...
	  $operator=$_POST['s_operator'];
		    ?>
			   ...
			   if (x=='brokd_is_wo' || x=='brokd_is_dispossed')
			      {
				    document.getElementById('op_gt').style.display='none';
					document.getElementById('op_lt').style.display='none';
					document.getElementById('op_gte').style.display='none';
			   ...
			   if (field=='brokh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/broken_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/broken_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/broken_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
			 }  
