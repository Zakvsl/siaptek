	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
                               ...
                               WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id' AND 
			...
			$q_page="SELECT COUNT(*) 
                     ...
                     INNER JOIN category_item ON master_item.cati_id=category_item.cati_id
					 INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                     WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id' AND $find_data";
		  }
	 }
  else
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no,itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
                               ...
                               INNER JOIN category_item ON master_item.cati_id=category_item.cati_id
							   INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                               WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id' 
  ...
  <table width="100%" border="0" cellspacing="1" cellpadding="1">
    ...
    <tr>
      ...
      <td align="right">Urut berdasarkan    : 
		  ...
		  <select id="s_sorting_field" name="s_sorting_field" onchange="submit()"> 
		   <?php 
		         if (isset($_POST['s_sorting_field']))
				    ...
				    {
					  ?>
					     <option value="itemd_code">Kode Tube</option>
		                 <option value="masti_name">Deskripsi Isi Aset</option>
		                 <option value="itemd_serial_no">Serial No</option>
		                 <option value="itemd_capacity">Kapasitas</option>
		                 <option value="cati_name">Kategori</option>
		...
		<input type="submit" id="btn_select" name="btn_select" value="Pilih"/>
    ...
    <tr>
	  ...
	  <table width="125%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="12%" scope="col" class="th_ltb">Kode Item</th>
          <th width="32%" scope="col" class="th_ltb">Nama Item</th>
        ...
        <tr>
	      ...
	      <?php
              ...
              $total_rows=mysqli_num_rows($query_exec);
              $maks_rows=ceil($total_rows/$row);
		  ?>	  
																								   
	<tr>
        <td align="left">Total : <?php echo $total_rows;?>&nbsp;data</td>
						      ...
						      function add_row(itemd_code, itemd_id, masti_name, itemd_serial_no, cati_name, masti_capacity, itemd_qty, whsl_name)	
                                       {
			                             ...
			                             newTD.align="center";
			                             newTD.innerHTML = '<input type="checkbox" id="cb_data[]" name="cb_data[]" value="'+itemd_id+'" />';
			                             ...
			                             newTD.align="left";
									     newTD.innerHTML ='<select id="s_old_item_name_"'+itemd_id+' name="s_old_item_name_'+itemd_id+'" style="width:300px"><option value="1">Deskripsi Isi Aset : '+masti_name+' | Isi : '+masti_capacity+' | Kategori : '+cati_name+'</option></select>';
									
									     newTD = newTR.insertCell(newTR.cells.length);  // qty
			                             newTD.align="left";
									     newTD.innerHTML ='<select id="s_new_item_name_'+itemd_id+'" name="s_new_item_name_'+itemd_id+'" style="width:300px"><?php
									                          $q_get_master_item='SELECT masti_id, masti_code, CONCAT("Deskripsi Isi Aset : ", masti_name," | Isi : ",
														      ...
														      while ($field_data=mysqli_fetch_array($exec_get_master_item))
														            {
														              echo '<option value="'.$field_data["masti_id"].'">'.$field_data["masti_name"].'</option>';
															        }	 
														      ?></select>';
			                             ...
			                             newTD.align="left";
			                             newTD.innerHTML ='<input type="checkbox" id="cb_cidd_is_canceled_'+itemd_id+'" name="cb_cidd_is_canceled_'+itemd_id+'" value="1" disabled="disabled"/> Tidak';
									
									     newTD = newTR.insertCell(newTR.cells.length);  // price			  
			                             newTD.align="left";
			                             newTD.innerHTML = '<input type="text" id="cidd_notes_'+itemd_id+'" name="cidd_notes_'+itemd_id+'" value="" size="35"/>';   
