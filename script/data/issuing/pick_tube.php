<?php
 ...
 if (isset($_GET['b']))
    {
	  $original_branch_id=$_GET['b'];
	  if ($branch_id!=$original_branch_id)
	     {
		   ?>
		     <script language="javascript">
			     alert('Ada perbedaan akses kantor cabang!\nSilahkan direfresh terlebih dahulu!');
			     window.close();
			 </script>
		   <?php
		 }
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
                               ...
                               WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND item_detail.branch_id='$branch_id' AND itemd_status='0' AND
							         warehouse_location.branch_id='$branch_id' AND $find_data LIMIT $first_data,$row";
			$q_page="SELECT COUNT(*) AS total_page
					 ...
					 INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                     WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND item_detail.branch_id='$branch_id' AND itemd_status='0' AND 
					       warehouse_location.branch_id='$branch_id' AND $find_data";
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
                               ...
                               WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id' 
							         AND warehouse_location.branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT COUNT(*) AS total_page
                     FROM item_detail
					 INNER JOIN uom ON uom.uom_id=item_detail.uom_id
                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
					 ...
					 INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                     WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id'
					       AND warehouse_location.branch_id='$branch_id'";
		  }
	   else 
	      {	  
	        ...
	        $q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
							   ...
							   INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                               WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND item_detail.branch_id='$branch_id' AND itemd_status='0' AND
							         warehouse_location.branch_id='$branch_id' AND $find_data $sorting 
	   ...
	   $q_show_pick_tube="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
							     ...
							     INNER JOIN warehouse_location ON warehouse_location.whsl_id=item_detail.whsl_id
                                 WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND item_detail.branch_id='$branch_id' AND itemd_status='0' AND
							           warehouse_location.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT COUNT(*) AS total_page 
                FROM item_detail
				INNER JOIN uom ON uom.uom_id=item_detail.uom_id
                INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                ...
                WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND itemd_status='0' AND item_detail.branch_id='$branch_id' AND
				      warehouse_location.branch_id='$branch_id'"; 
	 }	 
	  ...
	  if ($itemd_id!='')
	     {
	       $q_check_item="SELECT item_detail.itemd_id, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, cati_name, itemd_qty, whsl_name 
                          ...
                          WHERE itemd_is_broken='0' AND itemd_is_dispossed='0' AND itemd_position='Internal' AND item_detail.branch_id='$branch_id' AND 
					            warehouse_location.branch_id='$branch_id' AND item_detail.itemd_id IN ($itemd_id)";	 		    
						      ...
						      function add_row(itemd_code, itemd_id, masti_name, itemd_serial_no, cati_name, masti_capacity, itemd_qty, whsl_name)	
                                       {
									     ...
									     newTD = newTR.insertCell(newTR.cells.length);  // price			  
			                             newTD.align="left";
			                             newTD.innerHTML ='<select id="s_issuingd_status_'+itemd_id+'" name="s_issuingd_status_'+itemd_id+'" onChange="call_change_status('+itemd_id+')"><option value="0">Normal</option><option value="1">Rental</option><option value="2">UJM</option></select><input type="hidden" id="txt_issuingd_status_'+itemd_id+'" name="txt_issuingd_status_'+itemd_id+'" value="0">';
									
									     newTD = newTR.insertCell(newTR.cells.length);  // price			  
			                             newTD.align="left";
			                             newTD.innerHTML ='<input type="checkbox" id="cb_issuingd_is_canceled_'+itemd_id+'" name="cb_issuingd_is_canceled_'+itemd_id+'" value="1" disabled="disabled"/> Tidak';
									
									     newTD = newTR.insertCell(newTR.cells.length);  // price			  
			                             newTD.align="left";
			                             newTD.innerHTML = '<input type="text" id="issuingd_notes_'+itemd_id+'" name="issuingd_notes_'+itemd_id+'" value="" size="35"/>';   
