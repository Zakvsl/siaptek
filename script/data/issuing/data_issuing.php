...
<?php
  ...
  $array_field=array("issuingh_code", "issuingh_date", "issuingh_ba_no", "issuingh_po_no", "issuingh_do_no", "issuingh_type", "cust_name", "issuingh_status", "issuingh_is_canceled", "issuingh_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='issuingh_code')
	       $field_to_sort='issuingh_id';
	   else
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_issuing="SELECT issuingh_id	AS issuingh_id_1, issuingh_code, issuingh_date, issuingh_do_no, issuingh_ba_no, issuingh_po_no,
                             CASE issuingh_type
			...
			$q_page="SELECT count(issuingh_id) as total_page 
	                 FROM issuing_header
					 INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_issuing="SELECT issuingh_id	AS issuingh_id_1, issuingh_code, issuingh_date, issuingh_do_no, issuingh_ba_no, issuingh_po_no,
                             ...
                             FROM issuing_header
							 INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
                             WHERE issuing_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(issuingh_id) as total_page 
	                 FROM issuing_header
					 INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
...
<form id="f_issuing" name="f_issuing" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA PENGELUARAN ASET</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='issuingh_ba_no')
								     echo "Nomor Berita Acara";
								 else
							     if ($fields=='issuingh_po_no')
								     echo "Purchase No";
								 else
								 if ($fields=='issuingh_do_no')
								     echo "DO No";             
								 else
								 ...
								 if ($fields=='issuingh_status')
								     echo "Status";
							     else
								 if ($fields=='issuingh_is_canceled')
								     echo "Dibatalkan";
							     else
								 if ($fields=='issuingh_notes')
								     echo "Keterangan";
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="issuingh_ba_no">Nomor Berita Acara</option>
		  <option value="issuingh_po_no">Purchase No</option>
		  <option value="issuingh_do_no">DO No</option>
		  <option value="issuingh_type">Tipe</option>
		  <option value="cust_name">Nama Customer/Vendor</option>
		  <option value="issuingh_status">Status</option>
		  <option value="issuingh_is_canceled">Dibatalkan</option>
		  <option value="issuingh_notes">Keterangan</option>
        </select>
		<?php } 
		       ...
		       <input name="txt_date_2" type="text" id="txt_date_2"  readonly="readonly" size="10" 
		                                       <?php 
											      if (isset($_POST['txt_date_2']))
											          ...
											          echo "value='$current_date'";
											   ?>/></div>
        <input type="submit" name="btn_find" id="btn_find" value="Cari"/><div id="div_4" style="display:none">&nbsp;0=Tidak, 1=Ya</div><div id="div_5" style="display:none">&nbsp;0=Customer, 1=Vendor</div><div id="div_6" style="display:none">&nbsp;0=Belum Kembali, 1=Kembali Sebagian, 2=Kembali Semua</div>
      </div></td>
      <td width="21%" nowrap="nowrap">Urut Berdasarkan   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
											  ...
											  if ($field=='issuingh_ba_no')
								                  echo "Nomor Berita Acara";
								              else
											  if ($field=='issuingh_po_no')
								                  echo "Purchase No";
								              else
								              if ($field=='issuingh_do_no')
								                  echo "DO No";             
								              else
											  ...
											  if ($field=='issuingh_status')
								                  echo "Status";
							                  else
											  if ($field=='issuingh_is_canceled')
								                  echo "Dibatalkan";
							                  else
								              if ($field=='issuingh_notes')
								                  echo "Keterangan";
											?>
          </option>
          <?php    
...
<td width="9%" align="left"><?php 
                              if ($issuing_autho=='W' || $issuing_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="10%" scope="col" class="th_ltb">Nomor Berita Acara </th>
          <th width="7%" scope="col" class="th_ltb">Purchase No </th>
          <th width="7%" scope="col" class="th_ltb">DO No </th>
          <th width="5%" scope="col" class="th_ltb">Tipe</th>
          <th width="17%" scope="col" class="th_ltb">Nama Customer/Vendor</th>
          <th width="6%" scope="col" class="th_ltbr">Status</th>
          <th width="7%" scope="col" class="th_ltbr">Total Aset </th>
		  ...
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection,$q_show_issuing);
				  $total_issuing=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_issuing=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_issuing['issuingh_id_1'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><label style="cursor: pointer;" onclick="show_data('<?php echo $data_issuing['issuingh_id_1'];?>')"><font color="#0000FF"><?php echo $data_issuing['issuingh_code'];?></font></label></td>
          <td class="td_lb"><?php echo get_date_1($data_issuing['issuingh_date']);?></td>
          <td class="td_lb"><?php echo $data_issuing['issuingh_ba_no'];?></td>
          <td class="td_lb"><?php echo $data_issuing['issuingh_po_no'];?></td>
          <td class="td_lb"><?php echo $data_issuing['issuingh_do_no'];?></td>
          <td class="td_lb"><?php echo $data_issuing['issuingh_type'];?></td>
          <td class="td_lb"><?php echo $data_issuing['cust_name'];?></td>
          <td class="td_lbr"><?php echo $data_issuing['issuingh_status'];?></td>
          <td class="td_lbr"><?php 
		                      $q_get_total_tube="SELECT COUNT(*) AS total FROM issuing_detail WHERE issuingd_is_canceled='0' AND issuingh_id='".$data_issuing['issuingh_id_1']."'";                              $exec_get_total_tube=mysqli_query($db_connection,$q_get_total_tube);
							  $field_get_total_tube=mysqli_fetch_array($exec_get_total_tube);
							  if ($field_get_total_tube['total']>0)
							      echo $field_get_total_tube['total'];
							  else
							      echo '0';
							  ?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_issuingh_is_canceled_<?php echo $data_issuing['issuingh_id_1'];?>" name="cb_issuingh_is_canceled_<?php echo $data_issuing['issuingh_id_1'];?>" <?php if ($data_issuing['issuingh_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($issuing_autho!='D') echo "disabled='disabled'";?> onclick="cancel_issuing('<?php echo $data_issuing['issuingh_id_1'];?>')"><?php if ($data_issuing['issuingh_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_issuing['issuingh_id_1'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_issuing['issuingh_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_issuing.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/issuing/cru_issuing.php?c=i", "f_cru_issuing", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/issuing/cru_issuing.php?c=u&id='+value_id, 'f_cru_issuing', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 }  
  ...
  function show_data(id)
           {
			 ...
			 var l=(screen.width/2)-(w/2);
			 var t=(screen.height/2)-(h/2);
		     open_child=window.open('../data/issuing/cru_issuing.php?c=d&id='+id+'&b='+branch_id_data, 'f_cru_issuing', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
		   }
			 
	function cancel_issuing(x)
	         {
			    var branch_id_data=document.getElementById('s_branch_data').value;
			    var w=400;
			    var h=230;
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
				var a="cb_issuingh_is_canceled_"+x;
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/issuing/canceled_issuing.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_issuing", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/issuing/canceled_issuing.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_issuing", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='issuingh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/issuing/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/issuing/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/issuing/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
			 }
