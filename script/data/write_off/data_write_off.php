...
<?php
  ...
  $array_field=array("woh_code", "woh_date", "woh_sources", "brokh_code", "woh_is_canceled", "woh_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='woh_code')
	       $field_to_sort='woh_id';
	   else
  ...
  if (isset($_POST['btn_find']))
	 {
	   ...
	   if ($field=='woh_date' && $operator=='between' && ($text_to_find_2<$text_to_find_1))
	      {
			 $continue='1';
			 ?>
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_write_off="SELECT woh_id, woh_code, woh_date, 
                               CASE woh_sources
                               ...
                               WHERE write_off_header.branch_id='$branch_id'  AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(woh_id) as total_page 
	                 FROM write_off_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                     WHERE write_off_header.branch_id='$branch_id'  AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_write_off="SELECT woh_id, woh_code, woh_date, 
                               ...
                               FROM write_off_header
                               LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                               WHERE write_off_header.branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(woh_id) as total_page 
	                 FROM write_off_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
	      ...
	      {	  
	        ...
	        $q_show_write_off="SELECT woh_id, woh_code, woh_date, 
                               ...
                               FROM write_off_header
                               LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                               WHERE write_off_header.branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(woh_id) as total_page 
	                 FROM write_off_header
                     LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                     WHERE write_off_header.branch_id='$branch_id' AND $find_data";  
	   ...
	   $q_show_write_off="SELECT woh_id, woh_code, woh_date, 
                          ...
                          FROM write_off_header
                          LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                          WHERE write_off_header.branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(woh_id) as total_page 
	            FROM write_off_header
                LEFT JOIN broken_header ON broken_header.brokh_id=write_off_header.brokh_id AND broken_header.branch_id='$branch_id'
                WHERE write_off_header.branch_id='$branch_id'"; 
...
<form id="f_write_off" name="f_write_off" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA PENGHAPUSAN ASET</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="woh_code">No Transaksi</option>
		  <option value="woh_date">Tanggal</option>
		  <option value="woh_sources">Sumber Transaksi</option>
		  <option value="brokh_code">No Ref Kerusakan</option>
		  <option value="woh_is_canceled">Dibatalkan</option>
		  <option value="woh_notes">Keterangan</option>
        </select>
		<?php } 
	    ...
	    <?php }?>
        <div id="div_1" style="display:<?php if ($field!='woh_date') echo "inline"; else echo "none";?>"><input type="text" name="txt_find_1" id="txt_find_1"
		 value="<?php
		         if (isset($_POST['txt_find_1']))
				     ...
				     echo "";	    
		       ?>"></div>
        <div id="div_2" style="display:<?php if ($field=='woh_date') echo 'inline'; else echo 'none';?>">
		   <input name="txt_date_1" type="text" id="txt_date_1" readonly="readonly" size="10" 
											   ...
											   ?>/></div>
		<div id="div_3" style="display:<?php if ($field=='woh_date' && $operator=='between') echo 'inline'; else echo 'none';?>">
      ...
      <td width="21%" nowrap="nowrap">Urut Berdasarkan   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='woh_sources')
								                  echo "Sumber Transaksi";
								              else
								              if ($field=='brokh_code')
								                  echo "No Ref Kerusakan";             
								              else
								              if ($field=='woh_is_canceled')
								                  echo "Dibatalkan";
								              else
								              if ($field=='woh_notes')
								                  echo "Keterangan";
...
<td width="9%" align="left"><?php 
                              if ($write_off_autho=='W' || $write_off_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="19%" scope="col" class="th_ltb">No Transaksi </th>
          <th width="8%" scope="col" class="th_ltb">Tanggal</th>
          <th width="13%" scope="col" class="th_ltb">Sumber Transaksi</th>
          <th width="15%" scope="col" class="th_ltb">No Ref Kerusakan </th>
          <th width="7%" scope="col" class="th_ltbr">Dibatalkan</th>
          <th width="33%" scope="col" class="th_ltbr">Ketererangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_write_off);
				  $total_write_off=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_write_off=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_write_off['woh_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><a href="javascript:void(1)" onclick="show_pic_write_off('<?php echo $data_write_off['woh_id'];?>')"><?php echo $data_write_off['woh_code'];?></a></td>
          <td class="td_lb"><?php echo get_date_1($data_write_off['woh_date']);?></td>
          <td class="td_lb"><?php echo $data_write_off['woh_sources'];?></td>
          <td class="td_lb"><?php echo $data_write_off['brokh_code'];?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_woh_is_canceled_<?php echo $data_write_off['woh_id'];?>" name="cb_woh_is_canceled_<?php echo $data_write_off['woh_id'];?>" <?php if ($data_write_off['woh_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($write_off_autho!='D') echo "disabled='disabled'";?> onclick="cancel_write_off('<?php echo $data_write_off['woh_id'];?>')"><?php if ($data_write_off['woh_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_write_off['woh_id'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_write_off['woh_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_write_off.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/write_off/cru_write_off.php?c=i", "f_cru_write_off", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/write_off/cru_write_off.php?c=u&id='+value_id, 'f_cru_write_off', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 }  
			 
	function cancel_write_off(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
				var a="cb_woh_is_canceled_"+x;
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/write_off/canceled_write_off.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_write_off", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/write_off/canceled_write_off.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_write_off", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='woh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/write_off/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/write_off/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/write_off/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 }  
