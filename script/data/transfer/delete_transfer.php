<?php
 ...
 $q_check_transfer="SELECT transfer_header.*, MONTH(tth_date) AS month, YEAR(tth_date) AS year 
                    FROM transfer_header WHERE tth_id IN ($delete) AND branch_id='$branch_id' ORDER BY tth_date, created_time DESC";
 ...
 if (mysqli_num_rows($exec_check_transfer)>0)
    {
      ...
      $delete_direct_id='';
	  $not_delete_code='';
	  while ($field_data=mysqli_fetch_array($exec_check_transfer))
	        {
		      ...
		      if ($active_period!='OK')
		         {
		           ...
		           ?>
			          <script language="javascript">
						  ...
						  var y='<?php echo $field_data['tth_code'];?>';
				          alert(x+'\n'+y);
				      </script>
			       <?php
				   break;
		      ...
		      if ($field_data['tth_is_canceled']=='0')
				 {
			       if ($field_data['tth_status']=='0')
			          {
						$itemd_id="";
				        $messages='0';
						$q_check_receipt_transfer="SELECT * FROM receipt_transfer_header WHERE tth_id='".$field_data['tth_id']."'";
				        ...
				        if (mysqli_num_rows($exec_check_receipt_transfer)>0)
				           {
				             if ($not_delete_code=='')
				                 $not_delete_code=$field_data['tth_code'];
				             else
				                 $not_delete_code=$not_delete_code.", ".$field_data['tth_code'];  
				           }
				        else
				           {
				             if ($delete_id=='')
				                 $delete_id="'".$field_data['tth_id']."'";
				             else
				                 $delete_id=$delete_id.", '".$field_data['tth_id']."'";	
					       }
				      }
			       else
			          {
				 ...
				 {
				   if ($delete_direct_id=='')
				       $delete_direct_id="'".$field_data['tth_id']."'";
				   else
				       $delete_direct_id=$delete_direct_id.", '".$field_data['tth_id']."'";					    
			     }
		    }
	}
      ...
      if ($delete_direct_id!='')
         {
	       ...
	       $q_delete_detail="DELETE FROM transfer_detail WHERE tth_id IN ($delete_direct_id)"; 
	       $exec_delete_detail=mysqli_query($db_connection, $q_delete_detail);
	       if ($exec_delete_detail)
	          {
		        $q_delete_header="DELETE FROM transfer_header WHERE tth_id IN ($delete_direct_id)";
		        ...
		        else
			       {
	                 ...
	                 ?>
	                   <script language="javascript">
                          ...
                          window.location='../../index/index.php?page=transfer';
		               </script>
	                 <?php 		
				   }			
			  }
 ...
 if ($continue=='1')
	{	   	
	  if ($delete_id!='')
	     {
		   $tube_id_update="";
		   $q_get_tube_update="SELECT * FROM transfer_detail 
				               INNER JOIN transfer_header ON transfer_header.tth_id=transfer_detail.tth_id
				               WHERE branch_id='$branch_id' AND transfer_detail.tth_id IN ($delete_id) AND ttd_is_canceled='0' AND tth_is_canceled='0'
							   ORDER BY tth_date, created_time DESC ";
		   ...
		   if ($continue=="1")
		      {
			    $q_delete_detail="DELETE FROM transfer_detail WHERE tth_id IN ($delete_id)"; 
				$exec_delete_detail=mysqli_query($db_connection, $q_delete_detail);
				if ($exec_delete_detail)
				   {
				     $q_delete_header="DELETE FROM transfer_header
                                       WHERE tth_id IN ($delete_id)";
		             $exec_delete_header=mysqli_query($db_connection, $q_delete_header);
					 if ($exec_delete_header)
					    {
						  mysqli_commit($db_connection);
						     ...
						     { 
	            		       ?>
	              		         <script language="javascript">
							        ...
							        if (x!='')
					    		        alert('Transaksi berikut ini tidak dapat dihapus, karena sudah diteruskan ke transaksi Penerimaan Transfer :\n'+x);
                    		        window.location='../../index/index.php?page=transfer';
		          		         </script>
	            		       <?php
							 } 
