<?php
 ...
 $q_delete="DELETE FROM customer 
            WHERE cust_type='1' AND branch_id='$branch_id' AND cust_id IN ($delete) AND cust_id NOT IN (SELECT cust_id FROM issuing_header WHERE issuingh_type='1'  AND 
			      branch_id='$branch_id' )";
 $q_check="SELECT cust_code
           FROM customer
           WHERE cust_type='1' AND branch_id='$branch_id' AND cust_id IN ($delete) AND cust_id IN (SELECT cust_id FROM issuing_header WHERE issuingh_type='1'  AND 
		         branch_id='$branch_id' )";
 //echo $q_delete;
