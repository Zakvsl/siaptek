...
<?php
  ...
  $array_field=array("cust_code", "cust_name", "cust_address", "cust_status", "cust_phone", "cust_fax","cust_email");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                            ...
                            FROM customer
							WHERE cust_type='1' AND branch_id='$branch_id' AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
					 WHERE cust_type='1' AND branch_id='$branch_id' AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                                   ...
                                   END cust_status, cust_phone, cust_fax, cust_email
                            FROM customer 
							WHERE cust_type='1' AND branch_id='$branch_id' $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
					 WHERE cust_type='1' AND branch_id='$branch_id'"; 
		  }
	   else 
	      {	  
	        ...
	        $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                                   ...
                                   END cust_status, cust_phone, cust_fax, cust_email
                            FROM customer
							WHERE cust_type='1' AND branch_id='$branch_id' AND $find_data $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(cust_id) as total_page 
	                 FROM customer
				     WHERE cust_type='1' AND branch_id='$branch_id' AND $find_data";  
		  }			 
	   ...
	   $q_show_customer="SELECT cust_id, cust_code, cust_name, cust_address, 
                              ...
                              END cust_status, cust_phone, cust_fax, cust_email
                       FROM customer 
					   WHERE cust_type='1' AND branch_id='$branch_id' $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(cust_id) as total_page 
	            FROM customer
				WHERE cust_type='1' AND branch_id='$branch_id'"; 
	 }	
...
<form id="f_customer" name="f_customer" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA VENDOR</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="cust_code">Kode Vendor</option>
			      ...
			      { 
				    if (a=='1')
				       open_child=window.open('../data/vendor/cru_vendor.php?c=u&id='+value_id, 'f_cru_vendor', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
					    ...
					    open_child=window.open('../data/vendor/pic_vendor.php?id='+value_id, 'f_pic_vendor', 'scrollBars=Yes, height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_customer.action="../data/vendor/delete_vendor.php";
	...
	function call_export()
	         {
			   ...
			   open_child=window.open("../data/vendor/export_to_excel.php?f="+field+"&o="+operator+"&t="+text_to_find);
			 }  
