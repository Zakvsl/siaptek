...
<?php
  ...
  $array_field=array("branch_name", "itemd_code", "masti_name", "masti_capacity", "itemd_serial_no", "itemd_acquired_date", "cati_name", "itemd_status", "itemd_position", "itemd_is_broken", "itemd_is_wo", "itemd_is_dispossed", "whsl_name", "cust_name", "branch_name");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_item_detail="SELECT branch_name, item_detail.itemd_id, itemd_code, masti_name, masti_capacity, uom_id_1, 
			                           (SELECT uom_name FROM uom WHERE uom_id=uom_id_1) AS uom_id_1, itemd_serial_no, itemd_acquired_date, itemd_qty, uom_id_2, 
                                       (SELECT uom_name FROM uom WHERE uom_id=uom_id_2) AS uom_id_2, cati_name, 
							     ...
							     WHERE $itemd_status AND $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(item_detail.itemd_id) as total_page 
				     ...
				     WHERE $itemd_status AND $find_data";  	 
		  }
	 }
  else
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
	...
	//   $text_to_find_1=htmlspecialchars(trim($_POST['txt_find_1']));
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_item_detail="SELECT branch_name, item_detail.itemd_id, itemd_code, masti_name, masti_capacity, uom_id_1, 
                                 ...
                                 INNER JOIN customer ON customer.cust_id=item_detail.vend_id
								 INNER JOIN branch ON branch.branch_id=item_detail.branch_id
							     WHERE $itemd_status AND warehouse_location.branch_id=branch.branch_id $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(item_detail.itemd_id) as total_page 
	                 FROM item_detail 
                     INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
                     INNER JOIN category_item ON category_item.cati_id=master_item.cati_id 
                     ...
                     INNER JOIN customer ON customer.cust_id=item_detail.vend_id
					 INNER JOIN branch ON branch.branch_id=item_detail.branch_id
				     WHERE $itemd_status AND warehouse_location.branch_id=branch.branch_id "; 
		  }
	   else 
	      {	  
	        ...
	        $q_show_item_detail="SELECT branch_name, item_detail.itemd_id, itemd_code, masti_name, masti_capacity, uom_id_1, 
                                 ...
                                 INNER JOIN customer ON customer.cust_id=item_detail.vend_id
								 INNER JOIN branch ON branch.branch_id=item_detail.branch_id
							     WHERE $itemd_status AND warehouse_location.branch_id=branch.branch_id AND $find_data $sorting 
			...
			$q_page="SELECT count(item_detail.itemd_id) as total_page 
                     ...
                     INNER JOIN customer ON customer.cust_id=item_detail.vend_id
					 INNER JOIN branch ON branch.branch_id=item_detail.branch_id
				     WHERE $itemd_status AND warehouse_location.branch_id=branch.branch_id AND $find_data";  
		  }			 
	 }
  else	 
  	 {
	   $q_show_item_detail="SELECT branch_name, item_detail.itemd_id, itemd_code, masti_name, masti_capacity, uom_id_1, 
                                 ...
                                 INNER JOIN customer ON customer.cust_id=item_detail.vend_id
								 INNER JOIN branch ON branch.branch_id=item_detail.branch_id
							     WHERE $itemd_status AND warehouse_location.branch_id=branch.branch_id $ordered_by 
								 LIMIT $first_data,$row"; 
	   $q_page="SELECT count(item_detail.itemd_id) as total_page 
	            FROM item_detail 
                INNER JOIN master_item ON master_item.masti_id=item_detail.masti_id
...
<form id="f_item_detail_all" name="f_item_detail_all" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="50%" scope="col"><h2>DAFTAR ASET SEMUA CABANG</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="masti_capacity">Kapasitas</option>
...
<td width="9%" align="left"><?php 
    ...
    <tr>
	  ...
	  <table width="160%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="2%" scope="col" class="th_ltb">Umur</th>
          <th width="6%" scope="col" class="th_ltb">Kategori</th>
          ...
          <th width="5%" scope="col" class="th_ltb">Lokasi Gudang</th>
          <th width="10%" scope="col" class="th_ltb">Vendor</th>
          <th width="10%" scope="col" class="th_ltb">Pemilik Aset </th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
        ...
        <tr>
          ...
          <td class="td_lb"><?php echo $font.$data_item_detail['masti_capacity']." ".$data_item_detail['uom_id_1'].$font1;?></td>
          <td class="td_lb"><?php echo $font.$data_item_detail['itemd_qty']." ".$data_item_detail['uom_id_2'].$font1;?></td>
          ...
          <td class="td_lb"><?php echo $font.$data_item_detail['original_branch_name'].$font1;?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_item_detail_all.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
			   ...
			   if (field=='itemd_acquired_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/item_detail_all/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2+'&v='+status);
					   }
					else
					   open_child=window.open("../data/item_detail_all/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+'&v='+status); 
				  }
			   else 
			      open_child=window.open("../data/item_detail_all/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+'&v='+status);
			 }  
