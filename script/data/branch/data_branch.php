...
<?php
  ...
  $array_field=array("branch_code", "branch_name", "branch_is_headquarter", "branch_address");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_branch="SELECT branch_id, branch_code, branch_name, 
			                       CASE branch_is_headquarter
                                        WHEN '0' THEN 'Kantor Pusat'
                                        WHEN '1' THEN 'Kantor Cabang'
                                   END branch_is_headquarter,  branch_address
                            FROM branch
							WHERE $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(branch_id) as total_page 
	                 FROM branch
					 WHERE $find_data";  	 
		  }
	 }
  ...
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_branch="SELECT branch_id, branch_code, branch_name, 
			                       CASE branch_is_headquarter
                                        ...
                                        WHEN '1' THEN 'Kantor Cabang'
                                   END branch_is_headquarter,  branch_address
                            FROM branch $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(branch_id) as total_page 
	                 FROM branch"; 
		  }
	   else 
	   ...
	   $q_show_branch="SELECT branch_id, branch_code, branch_name, 
			                       CASE branch_is_headquarter
                                        ...
                                        WHEN '1' THEN 'Kantor Cabang'
                                   END branch_is_headquarter,  branch_address
                       FROM branch $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(branch_id) as total_page 
	            FROM branch"; 
	 }	

 //echo $q_show_branch."<br>";	
 //echo $q_page; 
...
<form id="f_branch" name="f_branch" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DAFTAR KANTOR CABANG</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
						         if ($fields=='branch_code')
								     echo "Kode Kantor Cabang";
								 else
								 if ($fields=='branch_name')
								     echo "Nama Kantor Cabang";
								 else
								 if ($fields=='branch_is_headquarter')
								     echo "Tipe";             
								 else
								 if ($fields=='branch_address')
								     echo "Alamat"; 
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field">
		  <option value="branch_code">Kode Kantor Cabang</option>
		  <option value="branch_name">Nama Kantor Cabang</option>
		  <option value="branch_is_headquarter">Tipe</option>
		  <option value="branch_address">Alamat</option>
        </select>
		<?php } 
      ...
      <td width="21%" align="right" nowrap="nowrap">Urut berdasarkan    :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='branch_is_headquarter')
								                  echo "Tipe";             
								              else
								              if ($field=='branch_address')
								                  echo "Alamat"; 
											?>
          </option>
          <?php    
...
<td width="9%" align="left"><?php 
                              if ($branch_autho=='W' || $branch_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="17%" scope="col" class="th_ltb">Kode Kantor Cabang</th>
          <th width="36%" scope="col" class="th_ltb">Nama Kantor Cabang</th>
          <th width="14%" scope="col" class="th_ltb">Tipe</th>
          <th width="28%" scope="col" class="th_ltbr">Alamat</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_branch);
				  $total_branch=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_branch=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_branch['branch_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_branch['branch_code'];?></td>
          <td class="td_lb"><?php echo $data_branch['branch_name'];?></td>
          <td class="td_lb"><?php echo $data_branch['branch_is_headquarter'];?></td>
          <td class="td_lbr"><?php echo $data_branch['branch_address'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_branch.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/branch/cru_branch.php?c=i", "f_cru_branch", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/branch/cru_branch.php?c=u&id='+value_id, 'f_cru_branch', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_branch.action="../data/branch/delete_branch.php";
					   document.f_branch.submit();
					 }  
				}
		   }   
</script>
