...
<?php
  ...
  $array_field=array("reth_code", "issuingh_code", "reth_date", "reth_ref_no", "reth_ba_no", "reth_po_no", "cust_name", "whsl_name", "reth_is_canceled", "reth_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='reth_code')
	      $field_to_sort='reth_id';
	   else
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_return="SELECT reth_id AS reth_id_1, reth_code, issuingh_code, reth_date, reth_ref_no, cust_name, whsl_name, 
			...
			$q_page="SELECT count(reth_id) as total_page 
	                 FROM return_header
                     INNER JOIN issuing_header ON issuing_header.issuingh_id=return_header.issuingh_id
					 INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
					 INNER JOIN warehouse_location ON warehouse_location.whsl_id=return_header.whsl_id
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_return="SELECT reth_id AS reth_id_1, reth_code, issuingh_code,  reth_date, reth_ref_no, cust_name, whsl_name, 
                            ...
                            INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
							INNER JOIN warehouse_location ON warehouse_location.whsl_id=return_header.whsl_id
                            WHERE return_header.branch_id='$branch_id' AND issuing_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' $sorting 
							LIMIT $first_data,$row"; 
			$q_page="SELECT count(reth_id) as total_page 
	                 FROM return_header
                     INNER JOIN issuing_header ON issuing_header.issuingh_id=return_header.issuingh_id
	      ...
	      {	  
	        ...
	        $q_show_return="SELECT reth_id AS reth_id_1, reth_code, issuingh_code, reth_date, reth_ref_no, cust_name, whsl_name, 
                            ...
                            INNER JOIN customer ON customer.cust_id=issuing_header.cust_id
							INNER JOIN warehouse_location ON warehouse_location.whsl_id=return_header.whsl_id
                            WHERE return_header.branch_id='$branch_id' AND issuing_header.branch_id='$branch_id' AND customer.branch_id='$branch_id' AND $find_data $sorting 
							LIMIT $first_data,$row"; 
			$q_page="SELECT count(reth_id) as total_page 
	                 FROM return_header
                     INNER JOIN issuing_header ON issuing_header.issuingh_id=return_header.issuingh_id
...
<form id="f_return" name="f_return" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA PENGEMBALIAN ASET</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='reth_ref_no')
								     echo "Ref No";             
								 else
								 if ($fields=='reth_ba_no')
								     echo "Nomor Berita Acara";             
								 else
								 if ($fields=='reth_po_no')
								     echo "Purchase No";             
								 else
								 if ($fields=='cust_name')
								 ...
								 if ($fields=='reth_is_canceled')
								     echo "Dibatalkan";             
								 else
								 if ($fields=='reth_notes')
								     echo "Keterangan";
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="reth_ref_no">Ref No</option>
		  <option value="reth_ba_no">Nomor Berita Acara</option>
		  <option value="reth_po_no">Purchase No</option>
		  <option value="cust_name">Nama Customer/Vendor</option>
		  <option value="whsl_name">Lokasi Gudang</option>
		  <option value="reth_is_canceled">Dibatalkan</option>
		  <option value="reth_notes">Keterangan</option>
        </select>
		<?php } 
...
<td width="9%" align="left"><?php 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="9%" scope="col" class="th_ltb">No Transaksi </th>
          <th width="8%" scope="col" class="th_ltb">No Ref Pengeluaran</th>
          <th width="7%" scope="col" class="th_ltb">Tanggal</th>
          <th width="7%" scope="col" class="th_ltb">Ref No</th>
          <th width="11%" scope="col" class="th_ltb">Nomor Berita Acara </th>
          <th width="9%" scope="col" class="th_ltb">Purchase No </th>
          <th width="13%" scope="col" class="th_ltb">Nama Customer/Vendor</th>
          <th width="6%" scope="col" class="th_ltbr">Total Aset </th>
          <th width="7%" scope="col" class="th_ltbr">Lokasi Gudang </th>
          <th width="6%" scope="col" class="th_ltbr">Dibatalkan</th>
          <th width="13%" scope="col" class="th_ltbr">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_return);
				  $total_return=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_return=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_return['reth_id_1'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><label style="cursor: pointer;" onclick="show_data('<?php echo $data_return['reth_id_1'];?>')"><font color="#0000FF"><?php echo $data_return['reth_code'];?></font></label></td>
          <td class="td_lb"><?php echo $data_return['issuingh_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_return['reth_date']);?></td>
          <td class="td_lb"><?php echo $data_return['reth_ref_no'];?></td>
          <td class="td_lb"><?php echo $data_return['reth_ba_no'];?></td>
          <td class="td_lb"><?php echo $data_return['reth_po_no'];?></td>
          <td class="td_lb"><?php echo $data_return['cust_name'];?></td>
          <td class="td_lbr"><?php 
		                          $q_get_total_tube="SELECT COUNT(*) AS total FROM return_detail WHERE retd_is_canceled='0' AND reth_id='".$data_return['reth_id_1']."'";
								  $exec_get_total_tube=mysqli_query($db_connection, $q_get_total_tube);
								  $field_total_tube=mysqli_fetch_array($exec_get_total_tube);
								  if ($field_total_tube['total']>0)
								      echo $field_total_tube['total'];
								  else 
		                              echo '0';
							 ?></td>
          <td class="td_lbr"><?php echo $data_return['whsl_name'];?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_reth_is_canceled_<?php echo $data_return['reth_id_1'];?>" name="cb_reth_is_canceled_<?php echo $data_return['reth_id_1'];?>" <?php if ($data_return['reth_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($returning_autho!='D') echo "disabled='disabled'";?> onclick="cancel_return('<?php echo $data_return['reth_id_1'];?>')"><?php if ($data_return['reth_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_return['reth_id_1'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_return['reth_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_return.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/return/cru_return.php?c=i", "f_cru_return", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/return/cru_return.php?c=u&id='+value_id, 'f_cru_return', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 }  
  ...
  function show_data(id)
           {
			 ...
			 var l=(screen.width/2)-(w/2);
			 var t=(screen.height/2)-(h/2);
		     open_child=window.open('../data/return/cru_return.php?c=d&id='+id+'&b='+branch_id_data, 'f_cru_return', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes'); 
		   }
	
	function cancel_return(x)
	         {
			    var branch_id_data=document.getElementById('s_branch_data').value;
			    var w=400;
			    var h=230;
				...
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/return/canceled_return.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_return", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/return/canceled_return.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_return", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='reth_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/return/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/return/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/return/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
			 }  
