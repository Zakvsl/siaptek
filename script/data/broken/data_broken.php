...
<?php
  ...
  $array_field=array("brokh_code", "brokh_date", "brokh_status", "brokh_is_canceled", "brokh_notes");
  ...
  if (isset($_POST['s_sorting_field'])) 
     {
	   if ($_POST['s_sorting_field']=='brokh_code')
	       $field_to_sort='brokh_id';
	   else
  ...
  if (isset($_POST['btn_find']))
	 {
	   ...
	   if ($field=='brokh_date' && $operator=='between' && ($text_to_find_2<$text_to_find_1))
	      {
			 $continue='1';
			 ?>
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_broken="SELECT brokh_id, brokh_code, brokh_date, 
                                   CASE brokh_status
			...
			$q_page="SELECT count(brokh_id) as total_page 
	                 FROM broken_header
...
<form id="f_broken" name="f_broken" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA ASET RUSAK </h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
				...
				foreach ($array_field as $fields)
				        {
						  ...
						  <?php	 
								 ...
								 if ($fields=='brokh_date')
								     echo "Tanggal";
								 else
								 if ($fields=='brokh_status')
								     echo "Status";
							     else
								 if ($fields=='brokh_is_canceled')
								     echo "Dibatalkan";
							     else
								 if ($fields=='brokh_notes')
								     echo "Keterangan";
						   ?>			 
						     </option>
						  <?php	 
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  <option value="brokh_code">No Transaksi</option>
		  <option value="brokh_date">Tanggal</option>
		  <option value="brokh_status">Status</option>
		  <option value="brokh_is_canceled">Dibatalkan</option>
		  <option value="brokh_notes">Keterangan</option>
        </select>
		<?php } 
		...
		<?php }?>
        <div id="div_1" style="display:<?php if ($field!='brokh_date') echo "inline"; else echo "none";?>"><input type="text" name="txt_find_1" id="txt_find_1"
		 value="<?php  
		         if (isset($_POST['txt_find_1']))
				     ...
				     echo "";	    
		       ?>"></div>
        <div id="div_2" style="display:<?php if ($field=='brokh_date') echo 'inline'; else echo 'none';?>">
		   <input name="txt_date_1" type="text" id="txt_date_1" readonly="readonly" size="10" 
											   ...
											   ?>/></div>
		<div id="div_3" style="display:<?php if ($field=='brokh_date' && $operator=='between') echo 'inline'; else echo 'none';?>">
		       <input name="txt_date_2" type="text" id="txt_date_2"  readonly="readonly" size="10" 
		                                       <?php 
											      if (isset($_POST['txt_date_2']))
											          ...
											          echo "value='$current_date'";
											   ?>/></div>
        <input type="submit" name="btn_find" id="btn_find" value="Cari"/><div id="div_4" style="display:none">&nbsp;0=Tidak, 1=Ya</div><div id="div_5" style="display:none">&nbsp;<a href="javascript:void(1)" onclick="call_information()">Informasi</a></div>
      </div></td>
      <td width="21%" nowrap="nowrap">Urut Berdasarkan   :
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php 
								              ...
								              if ($field=='brokh_status')
								                  echo "Status";
							                  else
								              if ($field=='brokh_is_canceled')
								                  echo "Dibatalkan";
							                  else
								              if ($field=='brokh_notes')
								                  echo "Keterangan";
											?>
          </option>
          <?php    
...
<td width="9%" align="left"><?php 
                              if ($broken_autho=='W' || $broken_autho=='D')
							     { 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="22%" scope="col" class="th_ltb">No Transaksi </th>
          <th width="6%" scope="col" class="th_ltb">Tanggal</th>
          <th width="22%" scope="col" class="th_ltbr">Status</th>
          <th width="8%" scope="col" class="th_ltbr">Dibatalkan</th>
          <th width="37%" scope="col" class="th_ltbr">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_broken);
				  $total_broken=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_broken=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_broken['brokh_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><label style="cursor: pointer;" onclick="show_data('<?php echo $data_broken['brokh_id'];?>')"><font color="#0000FF"><?php echo $data_broken['brokh_code'];?></font></label></td>
          <td class="td_lb"><?php echo get_date_1($data_broken['brokh_date']);?></td>
          <td class="td_lbr"><?php echo $data_broken['brokh_status'];?></td>
          <td class="td_lbr"><input type="checkbox" id="cb_brokh_is_canceled_<?php echo $data_broken['brokh_id'];?>" name="cb_brokh_is_canceled_<?php echo $data_broken['brokh_id'];?>" <?php if ($data_broken['brokh_is_canceled']=='Ya') echo "checked='checked' disabled='disabled'"; else if ($broken_autho!='D') echo "disabled='disabled'";?> onclick="cancel_broken('<?php echo $data_broken['brokh_id'];?>')"><?php if ($data_broken['brokh_is_canceled']=='Ya') echo "<a href='javascript:void(1)' onclick='display_reason(".$data_broken['brokh_id'].")'>Ya</a>"; else echo "Tidak";?></td>
          <td class="td_lbr"><?php echo $data_broken['brokh_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke: 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_broken.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/broken/cru_broken.php?c=i", "f_cru_broken", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0, scrollbars=Yes");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/broken/cru_broken.php?c=u&id='+value_id, 'f_cru_broken', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes');
				  }
			 }  
  ...
  function show_data(id)
           {
			 ...
			 var l=(screen.width/2)-(w/2);
			 var t=(screen.height/2)-(h/2);
		     open_child=window.open('../data/broken/cru_broken.php?c=d&id='+id+'&b='+branch_id_data, 'f_cru_broken', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0, scrollbars=Yes'); 
		   }
	
	function cancel_broken(x)
	         {
			    var branch_id_data=document.getElementById('s_branch_data').value;
			    var w=400;
			    var h=230;
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
				var a="cb_brokh_is_canceled_"+x;
				if (document.getElementById(a).checked==true)
			        open_child=window.open("../data/broken/canceled_broken.php?id="+x+'&a=c&b='+branch_id_data, "f_canceled_broken", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }
	...
	function display_reason(x)
	         {
			    ...
			    var l=(screen.width/2)-(w/2);
			    var t=(screen.height/2)-(h/2);
			    open_child=window.open("../data/broken/canceled_broken.php?id="+x+'&a=d&b='+branch_id_data, "f_canceled_broken", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0"); 
			 }	
			   ...
			   if (field=='brokh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/broken/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/broken/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/broken/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 }
	...
	function call_information()
	         {
			   alert('0 = Tidak Dihapus dan Tidak Dijual\n1 = Dihapus Sebagian\n2 = Dihapus Semua\n3 = Dijual Sebagian\n4 = Dijual Semua\n5 = Dihapus dan Dijual Sebagian\n6 = Dihapus dan Dijual Semua');
			   exit;
			 }
	function show_hide()
	         {
