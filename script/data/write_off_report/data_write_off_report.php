...
<?php
  ...
  $array_field=array("woh_code", "woh_date", "brokh_code", "itemd_code", "masti_name", "itemd_serial_no", "cati_name", "wod_is_canceled", "wod_notes");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_write_off_report="SELECT woh_code, woh_date, brokh_code, itemd_code, masti_name, itemd_serial_no, itemd_capacity, uom_name, 
                                     itemd_qty, cati_name, 
...
// echo $q_show_write_off_report."<br>";	
// echo $q_page; 
...
<form id="f_write_off_report" name="f_write_off_report" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>LAPORAN PENGHAPUSAN ASET </h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field" onchange="show_hide()">
		  ...
		  <option value="wod_is_canceled">Dibatalkan</option>
		  <option value="wod_notes">Keterangan</option>
        </select>
		<?php } 
...
<td width="9%" align="left"><div id="action" align="left">
                              <ul id="nav">
                                <li><a href="" title="Retrive write_off Report" onclick="call_export()">Ekspor Data</a></li>
                              </ul>
                            </div></td>
    </tr>
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="13%" scope="col" class="th_ltb">No Transaksi </th>
          ...
          <th width="7%" scope="col" class="th_ltb">Keterangan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_write_off_report);
				  $total_write_off_report=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_write_off_report=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_write_off_report['woh_code'];?></td>
          <td class="td_lb"><?php echo get_date_1($data_write_off_report['woh_date']);?></td>
          <td class="td_lb"><?php echo $data_write_off_report['brokh_code'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['itemd_code'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['masti_name'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['itemd_serial_no'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['itemd_capacity']." ".$data_write_off_report['uom_name'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['itemd_qty']." Cylinder";?></td>
          <td class="td_lb"><?php echo $data_write_off_report['cati_name'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['wod_is_canceled'];?></td>
          <td class="td_lb"><?php echo $data_write_off_report['wod_notes'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_write_off_report.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/write_off_report/cru_write_off_report.php?c=i", "f_cru_write_off_report", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/write_off_report/cru_write_off_report.php?c=u&id='+value_id, 'f_cru_write_off_report', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_write_off_report.action="../data/write_off_report/delete_write_off_report.php";
					   document.f_write_off_report.submit();
					 }  
				}
		   }   
  ...
  function active_inactive()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_write_off_report.action="../data/write_off_report/active_invactive_item.php";
					   document.f_write_off_report.submit();
					 }  
				}
		   }
			   ...
			   if (field=='woh_date')
			      {
				    if (operator=='between')
					   { 
					     ...
					     open_child=window.open("../data/write_off_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1+"&t2="+text_to_find_2);
					   }
					else
					   open_child=window.open("../data/write_off_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1); 
				  }
			   else 
			      open_child=window.open("../data/write_off_report/export_to_excel.php?f="+field+"&o="+operator+"&t1="+text_to_find_1);
			 } 
