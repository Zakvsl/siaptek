...
<?php
  ...
  $array_field=array("uom_code", "uom_name");
	   ...
	   if ($text_to_find_1=='')	  
	      {
			...
			$q_show_uom="SELECT uom_id, uom_code, uom_name
                            FROM uom
							WHERE $find_data LIMIT $first_data,$row"; 
			$q_page="SELECT count(uom_id) as total_page 
	                 FROM uom
					 WHERE $find_data";  	 
		  }
	 }
  ...
  if (isset($_POST['s_sorting_field']) || isset($_POST['s_sort']))
     {
	   ...
	   if ($text_to_find_1=='')
	      {
		    $q_show_uom="SELECT uom_id, uom_code, uom_name
                            FROM uom $sorting LIMIT $first_data,$row"; 
			$q_page="SELECT count(uom_id) as total_page 
	                 FROM uom"; 
		  }
	   else 
  	 ...
  	 {
	   $q_show_uom="SELECT uom_id, uom_code, uom_name
                            FROM uom $ordered_by LIMIT $first_data,$row"; 
	   $q_page="SELECT count(uom_id) as total_page 
	            FROM uom"; 
	 }	
...
<form id="f_uom" name="f_uom" method="post" action="">
  <table width="100%" border="0" cellspacing="1" cellpadding="2" class="table-list-home">
    <tr>
      <td width="69%" scope="col"><h2>DATA SATUAN ITEM</h2></td>
    ...
    <tr>
      <td scope="col"><div align="left">
        ...
        <select name="s_field" id="s_field">
		  <option value="uom_code">Kode Satuan</option>
		  <option value="uom_name">Nama Satuan Item</option>
        </select>
		<?php } 
        ...
        <select id="s_sorting_field" name="s_sorting_field" onchange="submit()">
          ...
          <?php    
			     ...
			     else
				    {
					  ?>
          <option value="uom_code">Kode Satuan</option>
          <option value="uom_name">Nama Satuan</option>
          <?php
					}		
		   ?>
        </select>
...
<td width="9%" align="left"><?php 
    ...
    <tr>
      <td colspan="3"><table width="100%" border="0" cellspacing="0" cellpadding="2" class="table-list">
        <tr bgcolor="#CCCCCC">
          ...
          <th width="21%" scope="col" class="th_ltb">Kode Satuan</th>
          <th width="74%" scope="col" class="th_ltb">Nama Satuan</th>
          </tr>
		  <?php
		     if ($continue=='0')
			    {
			 	  $exec_query=mysqli_query($db_connection, $q_show_uom);
				  $total_uom=mysqli_num_rows($exec_query);
				  $no=0;
				  while ($data_uom=mysqli_fetch_array($exec_query))
				        {
					      $no++;
   		  ?>
        <tr>
          <td align="center" class="td_lb"><input type="checkbox"  id ="check_data[]" name="check_data[]" value="<?php echo $data_uom['uom_id'];?>" /></td>
          <td align="center" class="td_lb"><?php echo $no;?></td>
          <td class="td_lb"><?php echo $data_uom['uom_code'];?></td>
          <td class="td_lb"><?php echo $data_uom['uom_name'];?></td>
          </tr><?php }} ?>
      </table></td>
    </tr>
	      <?php
	...
	<tr>
        ...
        <td colspan="2" align="right">Halaman Ke  : 
		                                                                               <select name="s_page" id="s_page" onchange="document.f_uom.submit()">
																								 <?php
																								      $pages=array();
																								      if ($maks_rows==0)
																								         {
	...
	function input_data()
	         {
			   ...
			   open_child=window.open("../data/uom/cru_uom.php?c=i", "f_cru_uom", "location=no,resizable=no, height="+h+", width="+w+", left="+l+", top="+t+", toolbar=0");
			 }	
	...
	function update_data()
	         {
			   ...
			   if (z>1)
			      ...
			      { 
				    open_child=window.open('../data/uom/cru_uom.php?c=u&id='+value_id, 'f_cru_uom', 'height='+h+', width='+w+', left='+l+', top='+t+', toolbar=0');
				  }
			 }  
  ...
  function delete_data()
           {
			 ...
			 if (z==0)
		     ...
		     else
			    {
				  ...
				  if (answer)
				     {
					   document.f_uom.action="../data/uom/delete_uom.php";
					   document.f_uom.submit();
					 }  
				}
		   }   
</script>
